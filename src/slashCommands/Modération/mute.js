  ...
  run: async (client, interaction) => {
    ...
    if (logs) {
      const embed = new EmbedBuilder()
        .setTitle("Mute")
        .setColor("Green")
        .setDescription(`Je viens de mute ${mutee.user.username} pendant ${timed} pour la raison : ${reason}`)
        .setTimestamp()
      await logs.send({ embeds: [embed] })
    }

    return interaction.reply({ content: `L'utilisateur ${mutee.user.username} a été mute pendant ${timed} pour la raison : ${reason}` })
