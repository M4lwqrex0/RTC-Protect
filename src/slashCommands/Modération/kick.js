...
module.exports = {
  ...
  run: async (client, interaction) => {
    if (!interaction.guild.members.me.permissions.has(PermissionsBitField.Flags.KickMembers)) return interaction.reply({ content: `Il me manque des permissions pour effectuer cette commande.`, ephemeral: true })
    if (!config.ownerID.includes(interaction.user.id) && !interaction.member?.permissions.has(PermissionsBitField.Flags.KickMembers)) return interaction.reply({ content: `Il te manque des permissions pour effectuer cette commande.`, ephemeral: true })
    ...
    if (logs) {
      const embed = new EmbedBuilder()
        .setTitle("Kick")
        .setColor("Green")
        .setDescription(`Je viens d'expuluser ${kickMember.user.username} pour la raison : ${reason}`)
        .setTimestamp()
